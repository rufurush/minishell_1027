...
/*   Updated: 2025/10/01 13:31:14 by kotadashiru      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	size_t  ft_substr_len(char const *s, unsigned int start, size_t len)
{
    ...
    s_len = ft_strlen(s);
    if(len == 0 || s_len <= start)//len == 1　の時s_len <= sで良いのか
        return(0);
...
static	char    *ft_store_substr(char const *s,char *result, unsigned int start, size_t len)
{
	size_t	substr_len;
	char	*tmp;
